...
def add_cart(request, product_id):
    ...
    # Parse the Variations from the POST request(Shared logic)
    ...
    if current_user.is_authenticated:
        ...
        if is_cart_item_exists:
            ...
            if product_variation in ex_var_list:
                ...
                item = CartItem.objects.create(user=current_user, quantity=1, product=product)
            ...
            # Create a brand new cart item if product doesn't exist in cart at all
...
def cart(request, total=0, quantity=0, cart_items=None):
    ...
    try:
        if request.user.is_authenticated:
            # If logged in, ONLY use the user ID to find items
